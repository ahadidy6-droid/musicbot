...
invite = "68123a17f1e83bfacffc1f46"

# Icecast server configuration
SERVER_HOST = "link.zeno.fm" # dont change.
SERVER_PORT = 80 # dont change
MOUNT_POINT = "/pjl2itzokawuv" # put ur mountpoint after / in ""
STREAM_USERNAME = "source" # dont change
STREAM_PASSWORD = "aHenYw0j"#"put your password"

vip_positions = {
                    "vi42342p": Position(5.5, 14.5, 4),
...
class SEA(BaseBot):
    ...
    async def on_chat(self, user: User, message: str):
        if message in vip_positions:
            try:
            # التحقق من امتيازات المستخدم (بحروف صغيرة)
                if user.username in vip_users:
                   # تنفيذ عملية النقل
                   target_position = vip_positions[message]
                   await self.highrise.teleport(user.id, target_position)
                  # تحديث موقع المستخدم بعد النقل
                   self.user_positions[user.username] = target_position
                else:
                   await self.highrise.send_whisper(user.id,"🚫 You do not have sufficient permissions to perform this operation.. tip the bot 1k")
            except Exception as e:
                error_msg = f"❌ حدث خطأ أثناء محاولة نقل المستخدم {user.username}: {e}"
...
"Type 'yes' to confirm else type 'no' to cancel.")
        ...
        if message.startswith("/help"):
            try:
                await self.highrise.send_whisper(user.id,"\nAVAILABLE COMMANDS:\n-p <song name> or -p <youtube url> - Play a song.\n-q - Display the next song in the queue.\n-skip - Skip current song.\n-skip [number] - Skip a song in the queue.")
                await asyncio.sleep(3)
                await self.highrise.send_whisper(user.id, "\n/top [number] - Places a song at number in the queue\n"
                                    "/nح - Display the currently playing song\n"
            ...
            try:    
                ...
                if len(parts) > 1 and parts[1].isdigit():
                    ...
                    if 0 <= adjusted_index < len(self.req_files):
                        ...
                        if not (user.username in vip_users or user.username == req_user):
                ...
                try:
                    while True:
                        if bot_instance.req_files:
                            ...
                            print(f"Streaming from playlist: {playlist[playlist_index]['title']}")
                            playlist_index += 1
                        else:
                            if not AUDIO_FILES:
                                continue
